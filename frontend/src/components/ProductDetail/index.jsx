...
import { BsLinkedin, BsTwitter, BsFacebook, BsWhatsapp, BsLink } from "react-icons/bs";
        ...
        <>
            <div className="container mx-auto p-6 flex gap-10 border-1 rounded-[30px] bg-gray-200">
                <div className="flex flex-col">
                    <img src={selectedImage} alt="Product" className="w-[400px] h-[300px] object-cover rounded-lg" />
                    <div className="flex mt-3 space-x-2">
                        {product.images.map((img, index) => (
                            <img
                                ...
                                src={img}
                                alt="Thumbnail"
                                className={`w-16 h-16 object-cover rounded-lg cursor-pointer border-2 ${selectedImage === img ? "border-orange-500" : "border-transparent"
                                    }`}
                ...
                <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                        ...
                        <span className="text-sm text-gray-400">👁 {product.viewed} Viewed</span>
                        <IconButton>
                            <AiOutlineHeart className="text-gray-500" size={20} />
                        </IconButton>
                        <span className="text-orange-500 cursor-pointer">Add to wishlist</span>
                    </div>

                    <div className="flex items-center mt-3">
                        <span className="text-orange-500 text-3xl font-bold">${product.price.toFixed(2)}</span>
                        <span className="text-gray-400 line-through text-xl ml-3">${product.oldPrice.toFixed(2)}</span>
                    ...
                    <div className="mt-6 flex items-center gap-3">
                        ...
                        <IconButton className="text-blue-600"><BsLinkedin /></IconButton>
                        <IconButton className="text-blue-400"><BsTwitter /></IconButton>
                        <IconButton className="text-blue-700"><BsFacebook /></IconButton>
                        <IconButton className="text-green-500"><BsWhatsapp /></IconButton>
                        <IconButton className="text-orange-500"><BsLink /></IconButton>
                    </div>
                </div>
            </div>
            <div className="mt-10">
            <div className="container flex flex-col lg:flex-row ">
                <div className="bg-orange-500 text-white p-6 rounded-lg w-full lg:w-1/4">
                    <h1 className="text-5xl font-bold">4.0 <span className="text-xl">/5</span></h1>
                        ...
                        {[186, 18, 12, 5, 2].map((count, index) => (
                            <div key={index} className="flex items-center">
                                ...
                                <div className="w-full bg-gray-200 rounded-full h-2">
                                    <div className={`h-2 bg-white rounded-full w-[${count}%]`}></div>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="flex-1 p-6">
                    <div className="flex border-b">
                        {["Description", "Specification", "Discussion", "Reviews (223)"].map((tab) => (
                            <button
                                key={tab}
                    ...
                    <div className="mt-4">
                        {activeTab === "Description" && (
                            <>
                                <h2 className="text-2xl font-semibold">See the best picture no matter where you sit</h2>
                                <p className="text-gray-600 mt-2">Lorem ipsum dolor sit amet, consectetur adipiscing elit...</p>
                            </>
                        )}
                        {activeTab === "Specification" && <p>Specification details here...</p>}
                        {activeTab === "Discussion" && <p>Discussion forum here...</p>}
                        {activeTab === "Reviews (223)" && <p>All customer reviews here...</p>}
                    </div>
                </div>
            </div>
            </div>
