...
const HotSale: React.FC = () => {
    ...
    const fetchFlashSales = async () => {
      try {
        ...
        if (response.data && Array.isArray(response.data)) {
          ...
          setError('No flash sales available');
        }
      } catch (err) {
  ...
  // Flatten all products from all flash sales
  const allProducts = flashSales.flatMap((flashSale) =>
  ...
  // Countdown: use the nearest endDate among all flash sales
  const nearestEndDate = allProducts.length
    ? allProducts.reduce(
        (min, p) =>
          new Date(p.flashSaleEndDate).getTime() < new Date(min).getTime()
            ? p.flashSaleEndDate
            : min,
        allProducts[0].flashSaleEndDate
      )
    : null;

  const saleEndTimeRef = useRef(
    nearestEndDate
      ? new Date(nearestEndDate).getTime()
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const timeLeft = Math.max(
        Math.floor((new Date(nearestEndDate).getTime() - Date.now()) / 1000),
        0
      );
    ...
    <div className="bg-gradient-to-r from-red-500 via-orange-500 to-yellow-500 pb-6 rounded-3xl shadow-xl p-6">
      ...
      >
        {allProducts.map((product, idx) => {
          const compareAt =
            product.variant.compareAtPrice ?? product.variant.price;
          const discount = Math.round(
            ((compareAt - product.flashPrice) / compareAt) * 100
          );
          return (
            <SwiperSlide key={product.variantId + idx}>
              <div className="productItem h-[450px] border-2 border-[rgba(0,0,0,0.1)] rounded-[20px] bg-[#f1f1f1] shadow-lg flex flex-col items-center relative overflow-hidden">
                ...
                <div className="info flex flex-col items-center justify-center text-center gap-1 mt-4 w-full px-4">
                  <h3 className="font-[700] text-lg text-orange-500">
                    {product.flashPrice.toLocaleString('en-US')}$
                  </h3>
